...
## Load Huang et al. datasets

```{r ghm_datasets, cache=TRUE, dependson="functions_data"}
...
## Load historic FAO-GMIA

```{r historic_fao_gmia, cache=TRUE, dependson="ghm_datasets"}
...
## Long-term trends between irrigated areas and irrigation water withdrawals

```{r plot_historic_fao_gmia, cache=TRUE, dependson="historic_fao_gmia", fig.height=3.3, fig.width=4}
...
```{r long_term_trends, cache=TRUE, dependson=c("functions_data", "historic_fao_gmia")}
...
```{r plot_long_term_trends, cache=TRUE, dependson="long_term_trends", fig.width=5.4, fig.height=4.3}
...
## Plot trend between irrigated areas and irrigation water withdrawals

```{r data_water, cache=TRUE, dependson="historic_fao_gmia"}
...
```{r plot_merged, cache=TRUE, dependson=c("data_water"), fig.height=3, fig.width=5.4, fig.cap="Scatterplots of irrigated areas reported by FAO-GMIA against irrigation water withdrawals. Each dot is a country."}
...
```{r plot_divided, cache=TRUE, dependson="data_water"}
...
```{r plot_evapotranspiration, cache=TRUE, dependson="plot_divided", fig.height=3, fig.width=5.4}
...
```{r plot_potevap, cache=TRUE, dependson="check_potevap", fig.height=2, fig.width=5.4}
...
```{r influence_efficiency, cache=TRUE, dependson=c("plot_divided", "data_water"), fig.height=2.3, fig.width=5.4}
...
```{r log10, cache=TRUE, dependson="data_water"}
...
```{r diagnostic, cache=TRUE, dependson=c("data_water", "log10"), fig.width=4.5, fig.height=7}
...
```{r plot_missing2, cache=TRUE, dependson="log10", fig.height=3, fig.width=4.5, fig.cap="Proportion of missing values in irrigation water withdrawal per dataset."}
...
```{r obtain_beta, cache=TRUE, dependson="conduct_lm", fig.width=5, fig.height=6.5}
...
## Compare our approach with estimates by GHM

```{r predict, cache=TRUE, dependson=c("conduct_lm", "data_water")}
...
```{r plot_predicted, cache=TRUE, dependson=c("predict", "conduct_lm", "data_water"), fig.height=6.5, fig.width=5.7, fig.cap="Validation of our approach. The black dots and the error bars show the range of irrigation water withdrawal values predicted from irrigated areas only. The colored dots show the irrigation water withdrawal values outputted by Global Hydrological Models (DBHM, Ho8, LPJmL, MPI-HM, PCR-GLOBWB, WaterGap) and FAO-based datasets (Aquastat, Liu et al. 2016)."}
...
```{r check_estimates, cache=TRUE, dependson=c("predict", "plot_predicted"),fig.height=3, fig.width=4}
...
```{r all_together, cache=TRUE, dependson="check_estimates", fig.height=2, fig.width=2.3}
...
```{r check_gh_fao, cache=TRUE, dependson="check_estimates", fig.height=5, fig.width=4.7}
...
## Can future irrigation water withdrawals be estimated as a function of irrigated areas?

```{r future_irrigation, cache=TRUE}
...
```{r plot_future_irrigation, cache=TRUE, dependson="future_irrigation", fig.height=6.5, fig.width=6}
...
```{r plot_future_current, cache=TRUE, dependson=c("future_irrigation", "plot_future_irrigation", "plot_predicted", "check_estimates"), fig.height=3.5, fig.width=5}
...
```{r run_model, cache=TRUE, dependson=c("define_model", "set_boot", "lookup", "transform_sample_matrix", "sample_matrix")}
...
```{r plot_uncertainty, cache=TRUE, dependson="arrange_output", fig.height=5, fig.width=2.5, fig.cap="Uncertainty in the empirical distribution of $r^2$."}
...
```{r merge_plots, cache=TRUE, dependson=c("plot_uncertainty", "plot_uncertainty_GHM", "run_model"), fig.height=4, fig.width=5.2}
...
```{r cumulative_r2, cache=TRUE, dependson="arrange_output", fig.height=2.2, fig.width=3.3, fig.cap="Cumulative empirical distribution for $r^2$."}
...
```{r scatterplots, cache=TRUE, dependson="arrange_output", fig.height=7, fig.width=5.3, fig.cap="Scatterplots of $r^2$ against the triggers' levels.", dev="pdf"}
...
# Irrigated areas vs irrigation water withdrawals at different scales

## Australia (scheme level), USA (State level) and Colorado (County level)
...
```{r plot_australian, cache=TRUE, dependson=australia.dt, fig.height=2, fig.width=4}
...
```{r australian_regressions, cache=TRUE, dependson="australia.dt", fig.height=2, fig.width=3}
...
```{r plot_all_australia, cache=TRUE, dependson=c("australian_regressions", "plot_australian"), fig.height=3.7, fig.width=5.6}
...
```{r australian_diagnostics, cache=TRUE, dependson="australian_regressions", fig.height=4.5, fig.width=3.5}
...
```{r boot_beta, cache=TRUE, dependson=c("australia.dt", "australian_regressions"), fig.height=2, fig.width=2.5}
...
```{r plot_beta, cache=TRUE, dependson="boot_beta", fig.height=2.2, fig.width=5.5}
...
```{r annex_water_productivity, cache=TRUE, dependson="data_water", fig.height=3, fig.width=5.4, echo=FALSE}
