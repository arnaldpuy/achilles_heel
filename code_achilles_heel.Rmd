...
In this sections we will read in and clean all eight irrigation water withdrawal datasets. We first load the Aquastat dataset (`table4`), then the Aquastat without missing values (`liu.dt`), the `.nc` files from @Huang2018b (PCR-GLOBWB, H08, LPMjL, WaterGap), and the `.nc` files from ISIMIP (DBHM, MPI-HM).
...
```{r plot_merged, cache=TRUE, dependson="merge_with_area", dev="tikz", fig.height=3.3, fig.width=5, fig.cap="Scatterplots of irrigated areas reported by FAO-GMIA against irrigation water withdrawals."}
...
```{r missing, cache=TRUE, dependson="log10", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r scatterplots, cache=TRUE, dependson="arrange_output", fig.height=7, fig.width=5.3}
