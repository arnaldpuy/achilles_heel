...
`get_nc_data` is tailored to specifically extract irrigation water withdrawal data at the country level from the datasets compiled by @Huang2018b. Note that inside the function there are calls to `country_code` and `coors2country` to transform each longitude and latitude value to a country name.
...
In the next code snippet we will read in and clean all eight irrigation water withdrawal datasets. We first load the Aquastat dataset (`table4`), then the Aquastat without missing values (`liu.dt`), the `.nc` files from @Huang2018b (PCR-GLOBWB, H08, LPMjL, WaterGap), and the `.nc` files from ISIMIP (DBHM, MPI-HM).

We show at the end of the code chunk that MPI-HM produced an outlier for Gabon. The difference with the values outputted by the other GHM is very large and suggests that it is actually a miscalculation. To prevent this outlier from biasing the computations, we transformed it to `NA` in order to impute it later on.
...
Here we read in the irrigated areas reported by FAO-GMIA. We used the data compiled by @Meier2018, and produce the `meier.dt` dataset.
...
Finally, we plot irrigated areas against irrigation water withdrawals. As shown in Figure 1 below, they are clearly related and their trend suggests that the former might actually describe very much the behaviour of the latter. We used this plot as preliminary evidence for the overparametrization of GHM, since a single parameter drives most of the variance in their model output. Given the shape of the scatterplots, the relation between irrigated area and irrigation water withdrawal can very well be modeled with a simple linear regression model.
...
In order to set the ground for the computations, which will be launched hereafter, we log-transform both irrigation water withdrawals and irrigated area values, and export all files created so far to `.csv`.
...
In multiple imputation methods, the proportion of missing values conditions the number of imputations required to obtain an appropriate estimate of the true missing value. @Graham2007, for instance, recommended 20 iterations for 20\%--30\% missing data, and 40 iterations for 50\% missing data. Which iteration to use in the regressions is also a source of uncertainty, which will be duly accounted for in due time. For now, we just calculate the fraction of missing values in each GHM and FAO-based dataset. Figure 2 below shows the results, and indicates that most datasets have c. 10-20\% of missing values. The fact that one dataset shows a much larger proportion of missing values (Aquastat in the Americas, c. 40\%) made us set the number of iterations at 40 to prevent loss of statistical power (see below).
...
The regressions just conducted allow to obtain the coefficients to predict irrigation water withdrawals from irrigated areas only, and account for the main sources of uncertainty in the regression. To check the efficiency of this approach, we predicted irrigation water withdrawal values from the irrigated areas reported by FAO-GMIA [@Siebert2013], and by five other irrigated area products: the IWMI-GIAM [@Thenkabail2009], the GRIPC [@Salmon2015], the Meier dataset [@Meier2018], Aquastat [@FAO2016b] and FAOSTAT [@FAO2017a]. We compared the resulting estimates with the irrigation water withdrawal estimates outputted by GHM and with those reported in FAO-based datasets.

Once the predictions were done, we created the `water.quantiles` dataset, which reports the min, median, max and several quantiles to describe the uncertainty in irrigation water withdrawals at the country level.
...
# Lookup table
Our model (see below) will run rowwise, for $i=1,2,...,N$ rows, and it will select an $r^2_i$ obtained given the conditions defined by three triggers: trigger $X_{1_i}$ will define the GHM or FAO-based product to retrieve the irrigation water withdrawal values from; $X_{2_i}$ the multiple imputation method to use to fill out missing values, and $X_{3_i}$ the iteration from the pool of 40 different iterations computed for each missing value.

In order to speed up these computations, we now create a lookup table with all possible $r^2$ values given all possible combinations for Continent, $X_1$, $X_2$ and $X_3$ (`lookup`). We sort `lookup` by all these elements through the use of the `setkey` function, which increases the speed of binary search.

We finally export all datasets created so far to `.csv` files.
...
## Sample matrix
Now we create the sample matrix, where each row represents a sample point and each column a trigger. We use the function `sobol_matrices` of the `sensobol` package [@Puyk]. In this case, since we will compute first, second, third and total-order indices using the @Jansen1999 estimators, we create an $\textbf{A}$, a $\textbf{B}$ matrix, and $k$ $\textbf{A}_{B} ^{(j)}$ matrices, where all columns come from $\textbf{A}$ except the $j$-th, which comes from $\textbf{B}$, for $k=3$ (the number of triggers). Overall, this leads to a total computational cost of $C=N(k+2)=2^{13}(3+2)=40960$ model runs per continent.

The matrices created with the `sobol_matrices` function use Sobol' Quasi-Random Number sequences [@Sobol1967; @Sobol1976], which are uniformly distributed in $[0,1]$. We will transform these sequences to their appropriate probability distributions in the next code snippet.
...
The model is simply a one-line function that runs in the sample matrix as follows: at the $i$-th row, it creates a vector with all the conditions set by $X_{1_i}, X_{2_i}, X_{3_i}$, and uses this vector to extract from the lookup table the $r^2_i$ value according to these conditions.
...
And in this last section, we conduct the sensitivity analysis. We first plot the triggers against $r^2$ values, which allows to map back the model output into each of the triggers' levels. The resulting plot is Fig. XX.
...
We finally compute the Sobol' indices using the `sobol_indices` function of the sensobol package [@Puyk], and bootstrap the results 1000 times. We also plot first, second, third and total-order indices in different plots, and export the main results.
